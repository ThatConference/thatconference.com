			...
			{#each activeEvents.online as event, i (i)}
				<li class="col-span-1 rounded-md shadow-sm">
					{#await canAddSession(event.id)}
						<div in:fade class="col-span-1 rounded-md shadow-sm">
							<div class="mx-auto w-full rounded-md bg-white p-4 shadow-sm">
								<div class="animate-pulse">
									<div class="flex-1 space-y-2">
										<div class="h-3 w-1/2 rounded bg-gray-400" />
										<div class="h-3 w-3/4 rounded bg-gray-300" />
									</div>
								</div>
							</div>
						</div>
					{:then accessResults}
						{#if accessResults}
							<div
								...
								<label for={event.name} class="flex h-full">
									{#if event.id !== eventSelected}
										<div
											class="flex flex-1 cursor-pointer items-center justify-between truncate rounded-md border border-gray-200 bg-white hover:bg-gray-50">
											<div class="flex-shrink-0">
												<img class="object-fit h-14 p-2" alt="" src={event.logo} />
											</div>
