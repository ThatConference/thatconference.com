<script>
	...
	function handleHoverClick() {
		if (activity.status === 'ACCEPTED') goto(`/sessions/${activity.id}`);
		else goto(`/sessions/edit/${activity.id}`);
	}
		...
		<div class="flex items-center px-4 py-4 sm:px-6">
			...
			<div>
				<div class="flex flex-col space-y-4">
					<ShellButton>
						<a
							href={`/sessions/edit/${activity.id}`}
					...
					<ShellButton>
						<a
							href={`/sessions/clone/?id=${activity.id}`}
							class="w-full px-8 py-1 text-sm font-medium leading-5 md:px-10 ">
							<span>
